        ...
        <div className="grid lg:grid-cols-2 gap-24 items-center">
          <div>
            ...
            <div className="flex flex-wrap gap-4">
              {payments.map((p, idx) => (
                <div key={idx} className="flex items-center gap-2 bg-white px-4 py-2 rounded-full border border-gray-100 shadow-sm text-[10px] font-bold uppercase tracking-wider text-dark">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {amenities.map((item, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                className="bg-white p-10 rounded-[24px] border border-gray-100 hover:border-primary/30 transition-all hover:shadow-xl group"
              >
