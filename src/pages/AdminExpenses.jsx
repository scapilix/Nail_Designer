  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8 relative">
      ...
      <div className="flex justify-between items-end">
        <div>
          <h2 className="font-serif text-4xl mb-2">Gestão de <i className="text-primary italic font-normal">Despesas</i></h2>
          <p className="text-gray-400 text-sm">Controlo de custos, faturação a fornecedores e saídas de caixa contínuas.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <motion.div className="bg-white rounded-[24px] p-6 border border-gray-100 shadow-sm flex items-center gap-6">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center bg-gray-50 text-dark"><DollarSign className="w-5 h-5" /></div>
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-1">Total (Sempre)</p>
              <p className="text-3xl font-serif font-bold text-dark">{total.toFixed(2)} €</p>
            </div>
          </motion.div>
          <motion.div className="bg-white rounded-[24px] p-6 border border-gray-100 shadow-sm flex items-center gap-6">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center bg-red-50 text-red-500"><TrendingDown className="w-5 h-5" /></div>
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-1">Mês Corrente</p>
              <p className="text-3xl font-serif font-bold text-red-500">{currentMonthTotal.toFixed(2)} €</p>
            </div>
          </motion.div>
          <motion.div className="bg-white rounded-[24px] p-6 border border-gray-100 shadow-sm flex items-center gap-6">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center bg-primary/20 text-primary"><Receipt className="w-5 h-5" /></div>
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-1">Comprovativos Anexos</p>
              <p className="text-3xl font-serif font-bold text-dark">{expenses.filter(e => e.receipt_url).length}</p>
            </div>
          </motion.div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
         ...
         <div className="flex-1 bg-white p-8 rounded-[32px] border border-gray-100 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
               ...
               <h3 className="font-serif text-2xl text-dark">Análise de <i className="text-primary italic font-normal">Categorias</i></h3>
            </div>
            {expenses.length === 0 ? (
               <p className="text-sm text-gray-400 italic">Sem dados financeiros para analisar.</p>
            ) : (
               <div className="space-y-4">
                  {categoryStats.slice(0, 5).map((stat, i) => (
                     <div key={i}>
                        <div className="flex justify-between items-end mb-2">
                           <span className="text-sm font-bold text-dark">{stat.name}</span>
                           <span className="text-sm text-gray-400 font-medium">{Number(stat.val).toFixed(2)} € <span className="text-xs">({stat.percentage.toFixed(0)}%)</span></span>
                        </div>
         ...
         <div className="w-full lg:w-1/3 bg-dark p-8 rounded-[32px] border border-dark shadow-xl text-white">
            <div className="flex items-center gap-3 mb-6">
            ...
            {expenses.length > 0 ? (() => {
               ...
               return (
                  <div className="pt-4 border-t border-white/10">
                     <div className="text-red-400 font-serif text-4xl font-bold mb-2">{highest.amount.toFixed(2)} €</div>
                     <div className="text-lg font-bold truncate">{highest.description}</div>
                     <div className="text-xs text-gray-400 mt-2 uppercase tracking-widest">{highest.category} &middot; {new Date(highest.date).toLocaleDateString()}</div>
                  </div>
               );
      ...
      <div className="bg-white rounded-[32px] border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-8 border-b border-gray-50 flex justify-between items-center gap-4 bg-gray-50/50">
          <div className="flex items-center gap-4 bg-white px-4 py-3 rounded-xl max-w-md shadow-sm border border-gray-100 flex-1">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Procurar despesa..." className="bg-transparent border-none outline-none text-sm w-full" />
          </div>
          <span className="text-xs font-bold uppercase tracking-widest text-gray-400 px-4 py-2 bg-white rounded-full border border-gray-100 shadow-sm">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-dark/60 backdrop-blur-sm px-4">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white rounded-[32px] shadow-2xl w-full max-w-xl p-8 max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-8">
                <h3 className="font-serif text-2xl">{editTarget ? 'Editar' : 'Nova'} <i className="text-primary font-normal italic">Despesa</i></h3>
              ...
              <form onSubmit={handleSave} className="space-y-6">
                <div className="grid grid-cols-2 gap-6">
                   <div className="space-y-2 col-span-2">
                     <label className="text-xs font-bold uppercase tracking-widest text-gray-500 block">Descrição/Fornecedor</label>
                     <input required type="text" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Ex: Fatura EDP Janeiro" className="w-full bg-secondary border-none rounded-xl px-4 py-3 text-sm outline-none focus:ring-1 focus:ring-primary" />
                   </div>
                   
                   <div className="space-y-2 col-span-2">
                     <label className="text-xs font-bold uppercase tracking-widest text-gray-500 block">Categoria Analítica</label>
                   ...
                   <div className="space-y-2">
                     <label className="text-xs font-bold uppercase tracking-widest text-gray-500 block">Data Efetiva</label>
                     <input required type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className="w-full bg-secondary border-none rounded-xl px-4 py-3 text-sm outline-none focus:ring-1 focus:ring-primary" />
                   </div>
                   <div className="space-y-2">
                     <label className="text-xs font-bold uppercase tracking-widest text-gray-500 block">Valor (€)</label>
                     <input required type="number" min="0" step="0.01" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} className="w-full bg-secondary border-none rounded-xl px-4 py-3 text-sm outline-none focus:ring-1 focus:ring-primary" placeholder="0.00" />
                   </div>
                   ...
                   <div className="space-y-2 col-span-2 mt-2 pt-4 border-t border-gray-50">
                      <label className="text-xs font-bold uppercase tracking-widest text-gray-500 flex items-center gap-2 mb-3">
                         <Receipt className="w-4 h-4"/> Comprovativo de Pagamento <span className="text-[10px] bg-secondary px-2 py-0.5 rounded-md text-gray-400 font-medium">Opcional</span>
                      </label>
                      <div className="relative group cursor-pointer w-full h-24 rounded-2xl overflow-hidden border-2 border-dashed border-gray-300 hover:border-primary transition-colors flex flex-col items-center justify-center bg-secondary">
                        {previewReceipt ? (
                           <div className="flex flex-col items-center justify-center w-full h-full relative z-10 group-hover:opacity-40 transition-opacity">
                              ...
                              <span className="text-[10px] font-bold text-dark uppercase tracking-widest">Documento Anexado</span>
                           </div>
                        ) : (
                           <div className="flex flex-col items-center justify-center w-full h-full relative z-10">
                              <Upload className="w-6 h-6 text-gray-400 group-hover:text-primary transition-colors mb-2" />
                              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Digitalize ou carregue a fatura</span>
                           </div>
                        )}
                ...
                <div className="mt-8 pt-6 border-t border-gray-50 flex gap-4 justify-end">
                  ...
                  <button type="submit" disabled={uploading} className="bg-red-50 text-red-600 hover:bg-red-500 hover:text-white px-8 py-3 rounded-xl text-sm font-bold uppercase tracking-widest transition-all shadow-sm shadow-red-500/10 disabled:opacity-50 border border-red-100">
